...
public class CookbookServiceLogic implements CookbookService {
	...
	public List<Cookbook> findAllCookbooks() {
		return this.cookbookStore.retrieveAll();
	}

	@Override
	public Cookbook findCookbook(int cookbookId) {
		
		Cookbook cookbook = this.cookbookStore.retrieve(cookbookId);
		
		List<Recipe> list = this.recipeStore.retrieveAll(cookbook.getId());
